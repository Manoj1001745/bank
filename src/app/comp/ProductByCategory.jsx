...
function ProductByCat(props) {
  ...
  return (
   <section className=''>
    <div className="container mx-auto border rounded-2xl m-4 border-gray-200 py-6 px-4">
      {/* Category Header */}
      <div className='category-header flex items-center rounded-xl mb-6 overflow-hidden shadow-lg'>
        <h3 className="text-2xl font-bold text-black px-6 py-4 flex items-center gap-3">
          <span className="w-1 h-8 bg-red-400 rounded-full"></span>
          {props.title}
        </h3>
        <div className="flex-1"></div>
        <Link href={`/category/${props.id}`} className="text-black/80 hover:text-red-500 px-6 py-4 text-lg font-medium transition-colors">
          View All →
        </Link>
      </div>
      
      {/* Posts Grid */}
      <div className="box-11 flex py-6 gap-6">
